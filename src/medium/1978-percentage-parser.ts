...
type Sign<A> = A extends `${infer F}${string}` ? F extends '+' | '-' ? F : '' : ''
type Percent<A> = A extends `${string}%` ? '%' : ''
type PercentageParser<A extends string> = [Sign<A>, A extends `${Sign<A>}${infer N}${Percent<A>}` ? N : never, Percent<A>]
